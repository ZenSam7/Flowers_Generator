...
class CGAN:
    def __init__(self):
        ...
        self.cgan_model = Model(
            [self.latent_space_inp, self.label_inp], self.dis_gen_z, name="CGAN"
        )
        ...
        self.optimizer_dis = Adam(
            1e-5
        )  # У Дискриминатора меньше (чтоб не душил Генератор)
    ...
    def build_generator(self) -> Model:
        ...
        for i in range(6, 8):
            ...
            # x_temp = Dense(2**i)(x)
            x = Conv2DTranspose(
                2**i, (3, 3), activation=LeakyReLU(0.1), padding="same"
            )(x)
            x = Conv2DTranspose(
                2**i, (3, 3), activation=LeakyReLU(0.1), padding="same"
            )(x)
        ...
        self.generator = Model(
            [self.latent_space_inp, self.label_inp], x, name="generator"
        )
    ...
    def build_discriminator(self) -> Model:
        ...
        self.discriminator = Model(
            [self.image_inp, self.label_inp], x, name="discriminator"
        )
    ...
    def train(self, epochs, batch_size=128, sample_interval=100):
        ...
        for iter_learn in range(epochs):
            ...
            with tf.GradientTape() as dis_tape:
                ...
                l_dis = 0.5 * (
                    tf.reduce_mean(-tf.math.log(dis_real_output + 1e-8))
